...
fn test_channel_codegen() {
    ...
    assert!(code.contains("Event"), "Expected 'Event' struct in generated code");
    assert!(code.contains("event_type"), "Expected 'event_type' field");
    assert!(code.contains("QueryRequest"), "Expected 'QueryRequest' struct");
    assert!(code.contains("QueryResponse"), "Expected 'QueryResponse' struct");

    // Connection struct is generated with channel fields
    assert!(code.contains("Connection"), "Expected Connection struct");
    assert!(code.contains("events"), "Expected 'events' field in Connection");
    assert!(code.contains("query"), "Expected 'query' field in Connection");
    assert!(code.contains("ReceiveChannel"), "Expected ReceiveChannel type for server push");
